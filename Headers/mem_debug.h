...
result
Debug_Catch(result res_In, char* call_In, int32 line_In, char* file_In)
{
...
#define DBG(func) Debug_Catch(func, #func, line, file)
#else
...
#if defined(MEM_ALLOC_H) || defined(USE_MEM_ALLOC) || defined(USE_MEM_ALL)
#include "mem_alloc.h"
...
l_mem_alloc_printInfo(void)
{
    if(l_AllocHead)
    {
        ...
        for(mem_block* block = l_AllocHead; block; block = block->pNext, index++)
        {
            ...
            Mem_CheckBounds(((void*)block) + SIZE_META);
...
#if defined(MEM_POOL_H) || defined(USE_MEM_POOL) || defined(USE_MEM_ALL)
#include "mem_pool.h"
