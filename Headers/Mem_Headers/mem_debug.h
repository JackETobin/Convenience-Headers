...
// TODO: Mem debug functions should return a buffer filled with an error report and not do its own printing.
    ...
    char*           unit_Out)
{
    ...
    {
        char* status = null;
        status = (status_In & ALLOC_STATUS_READY) ? "READY" : status;
        status = (status_In & ALLOC_STATUS_ISSUE) ? "ISSUE" : status;
        status = (status_In & ALLOC_STATUS_FREED) ? "FREE" : status;
    ...
    l_mem_alloc_printInfo(void)
    {
        if(l_AllocHead)
        {
            ...
            for(mem_block* block = l_AllocHead; block; block = block->pNext, index++)
            {
                ...
                netSize += sizeBytes; count += (block->status != ALLOC_STATUS_FREED);
            }
