...
#define Mem_Alloc(size_In, tag_In, mem_Out) \
    DBG(_Mem_Alloc(size_In, tag_In, mem_Out))
...
#define Mem_Alloc_Allign(size_In, align_In, tag_In, mem_Out) \
    DBG(_Mem_Alloc_Allign(size_In, align_In, tag_In, mem_Out))
    ...
    const void**    mem_Out)
{
    ...
    Mem_Zero(block, size_In + SIZE_META);
    ...
    uint8           retNull_In)
{
    ...
    if(!blockNew) 
        return MEM_NOALLOC;
    Mem_Zero(blockNew, SIZE_META + size_In + sizeof(uint64));
