...
#define POOL_ALIGN 8
    ...
    #include "mem_alloc.h"
    #define Pool_Alloc(size_In, tag_In, mem_Out) Mem_Alloc_Allign(size_In, POOL_ALIGN, tag_In, (p_mem)&mem_Out)
    #define Pool_Realloc(size_In, mem_InOut)     Mem_Realloc(size_In, (p_mem)&mem_InOut)
    #define Pool_Free(mem_In)                    Mem_Free((p_mem)&mem_In)
    #define Pool_Free_All()                      Mem_Kill();
    ...
    uint64          size_In)
{
    ...
    res_hnd newResHnd = 0;
    result checkAll = Pool_Reserve_At(size_In, target->hnd, &newResHnd);
    if(checkAll != POOL_SUCCESS)
        return checkAll;

    uint64 range = res[1] * POOL_SECTION_SIZE;
    checkAll = Pool_Write(newResHnd, (void*)(res + 2), range, 0);
    if(checkAll != POOL_SUCCESS)
        return checkAll;

    checkAll = Pool_Release(*resHnd_InOut);
    if(checkAll != POOL_SUCCESS)
        return checkAll;
